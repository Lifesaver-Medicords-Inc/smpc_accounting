...
{
    public partial class OutputVatPage : UserControl
    {
        ...
        public OutputVatPage()
        {
            InitializeComponent();

            Helpers.Placeholder.SetPlaceholder(txt_search, placeHolderText);
            // AllowOnlyNumbers(txt_code) was here - wrong for an alphanumeric TAX
            // CODE field. See SetEditableColumns for the matching ReadOnly fix -
            // same bug found on InputVatPage, the screen this was cloned from.
        }
        ...
        private void SetEditableColumns(bool isEdit, bool isNewMode = false)
        {
            ...
            // txt_code/txt_tax_desc were permanently ReadOnly=true in the Designer -
            // unlocked for NEW records only, same fix as InputVatPage.
        ...
        private async Task<bool> ValidateTaxSetupAsync()
        {
            ...
            // 4. This screen only ever shows sales-side codes (see LoadTaxSetups) -
            // a code saved here without a Sales COA would immediately vanish from
            // its own list, which reads as data loss even though nothing was
            // actually lost. Require Sales specifically, not "either", on this page.
            bool hasSales = cmb_coa_sales.SelectedIndex != -1 && !string.IsNullOrWhiteSpace(cmb_coa_sales.Text);

            if (!hasSales)
            {
                Helpers.ShowDialogMessage("error", "Please select an Output Tax Account (Sales) - this screen only manages sales-side codes.");
                return false;
            }

            // 5. Validate unique Tax Code (NEW MODE ONLY) - against every tax code
            // that exists, not just this page's sales-filtered _taxTable, or a code
            // already in use on the purchase side would slip through undetected.
            if (_isNewMode)
            {
                string enteredCode = txt_code.Text.Trim();

                if (_taxdata?.tax_setup_view != null &&
                    _taxdata.tax_setup_view.Any(v =>
                        v.code?.Equals(enteredCode, StringComparison.OrdinalIgnoreCase) == true))
                {
        ...
        private async void OutputVatPage_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        // Phase 3 item 3.2: this is the same generic §4.5.3 Tax Code Setup screen
        // InputVatPage already is (that page shows every code, unfiltered - "Input
        // VAT" is just its current menu label, not an actual filter) - narrowed here
        // to the codes an OUTPUT (sales-side) tax account has actually been assigned
        // to, since coa_sales_id/coa_purch_id are already mutually exclusive per code
        // (see UpdateCoaComboState below - picking one disables the other), a real,
        // already-enforced distinction rather than an invented one.
        private async Task LoadTaxSetups()
        {
            _taxdata = await taxSetupService.GetAsModel();

            var outputCodes = _taxdata?.tax_setup_view?.Where(v => v.coa_sales_id != 0).ToList()
                ?? new List<TaxViewModel>();

            _taxTable = Helpers.ToDataTable(outputCodes);
