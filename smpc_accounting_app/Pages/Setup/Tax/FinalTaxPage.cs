...
{
    public partial class FinalTaxPage : UserControl
    {
        ...
        public FinalTaxPage()
        {
            ...
            // CODE field, same bug found on InputVatPage. Moot for this screen
            // specifically either way, since txt_code is fixed to FINAL_TAX_CODE
            // below rather than user-typed - kept ReadOnly always, unlike Input/
            // Output VAT's clones which unlock it for New.
        }
        ...
        private void SetEditableColumns(bool isEdit, bool isNewMode = false)
        {
            ...
            // TAX DESCRIPTION was permanently ReadOnly=true in the Designer (same bug
            // as InputVatPage) - unlocked for NEW records only. TAX CODE stays fixed
            // to FINAL_TAX_CODE (see btn_new_Click) - this screen manages exactly one
            // code, never a user-typed one.
        ...
        private void btn_new_Click(object sender, EventArgs e)
        {
            ...
            // This screen manages exactly one code - pre-fill and lock it so New
            // can never accidentally create a differently-coded row that would just
            // vanish from view (LoadTaxSetups filters to FINAL_TAX_CODE only).
            txt_code.Text = FINAL_TAX_CODE;
        ...
        private async void FinalTaxPage_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        // §4.5.3 lists ten tax codes; Final Tax is confirmed with the user to be an
        // eleventh, plain Tax Code Setup entry rather than a separate concept - this
        // screen is the same generic §4.5.3 screen InputVatPage already is, narrowed
        // to just that one code so it isn't lost in the other ten. FINAL_TAX_CODE is
        // the exact string this filters on and the value a "New" record here should
        // be saved with.
        private const string FINAL_TAX_CODE = "FINAL-TAX";

        private async Task LoadTaxSetups()
        {
            _taxdata = await taxSetupService.GetAsModel();

            var finalTaxCodes = _taxdata?.tax_setup_view?
                .Where(v => string.Equals(v.code, FINAL_TAX_CODE, StringComparison.OrdinalIgnoreCase))
                .ToList() ?? new List<TaxViewModel>();

            _taxTable = Helpers.ToDataTable(finalTaxCodes);
